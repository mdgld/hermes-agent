...
Provider switching uses the ``--provider`` flag exclusively, except for
the reserved ``moa:`` prefix that explicitly selects a Mixture-of-Agents
preset. Other colons are reserved for OpenRouter variant suffixes
(``:free``, ``:extended``, ``:fast``).
"""
...
def switch_model(
    ...
    # Reserved explicit MoA syntax. This must run before PATH B so
    # ``moa:default`` is never treated as an aggregator vendor:model slug,
    # while bare ``default``/``moa`` can no longer silently activate MoA.
    if new_model.lower().startswith("moa:"):
        explicit_provider = "moa"
        new_model = new_model.split(":", 1)[1].strip()
    ...
    if explicit_provider:
        ...
        if target_provider == "moa" and not new_model:
            ...
            new_model = resolve_default_moa_preset_name()
