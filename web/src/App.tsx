...
export default function App() {
  ...
  useEffect(() => {
    // Expand Telegram Mini App to full screen if available
    const tg = (window as any).Telegram?.WebApp;
    // tg exists and is an object, and version exists
    if (tg && typeof tg.version === "string") {
      setIsTelegram(true);
  ...
  return (
    <div 
      className="flex flex-col bg-background text-foreground overflow-x-hidden" 
      style={isFullHeight ? { height: "var(--tg-viewport-stable-height, var(--tg-viewport-height, 100dvh))", overflow: "hidden" } : { minHeight: "var(--tg-viewport-stable-height, var(--tg-viewport-height, 100dvh))" }}
    >
      ...
      <header 
        ...
        style={{ 
          paddingTop: isTelegram ? "max(env(safe-area-inset-top, 0px), 84px)" : "env(safe-area-inset-top, 0px)" 
        }}
      >
      ...
      <main
        key={animKey}
        className={`relative z-2 mx-auto w-full max-w-[1400px] ${isFullHeight ? "flex-1 overflow-hidden" : "flex-1 px-6 py-8"}`}
      ...
      <footer 
        className="relative z-2 border-t border-border"
