...
Skins are **pure data** — no code changes needed. Files: `hermes_cli/skin_engine.py` (built-in skins) + `~/.hermes/skins/*.yaml` (user skins). Missing values inherit from the `default` skin. Activate with `/skin <name>` or `display.skin: <name>`. Built-in skins: `default`, `ares`, `mono`, `slate`.
